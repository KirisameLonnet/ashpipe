...
/plugin marketplace add KirisameLonnet/ashpipe
/plugin update ashpipe@ashpipe
```

If update still reports an old version, uninstall and reinstall:

```
/plugin remove ashpipe@ashpipe
/plugin install ashpipe@ashpipe
```
...
- **SessionStart hook** — auto-detects `.ashpipe/` in the workspace and runs a time-bounded, best-effort `ashpipe mount`; unavailable portals never block Claude Code startup
...
Claude Code auto-discovers the plugin skill from `skills/ashpipe-mount` and
loads the hook from `hooks/hooks.json`.
